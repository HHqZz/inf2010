...
public class FormValidator
{
	...
	private static String generateCode(int length)
	{
		...
		{
			int  n = generator.nextInt(26) + 1;	// 26 en maximum pour les 26 lettres de l'alphabet
			charKey[i] = (char)(n + 64); 	// 64 car A est a la 65ieme position dans la table ASCII
			code=code + String.valueOf(charKey);
	...
	private static int[] generateTransform(int nb)
	{
		...
		for (int i = 0 ; i<nb ; i++ )
		{
			int  n = generator.nextInt(9);		// On genere un nb aleatoire entre 0 et 9
			charTransform[i]= n;				// On remplit le tableau avec les nb aleatoires
