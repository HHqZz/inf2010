...
public class PixelMap 
{
	...
	protected void AllocateMemory(ImageType type, int h, int w)
	{
		...
		// compl�ter
		if (type==ImageType.BW){
			BWPixel[][] imageBW = new BWPixel[height][width];
			imageData = imageBW;
		} else if (type==ImageType.Color){
			ColorPixel[][] imageColor = new ColorPixel[height][width];
			imageData = imageColor;
		} else if (type==ImageType.Gray){
			GrayPixel[][] imageGray = new GrayPixel[height][width];
			imageData = imageGray;
		} else if (type==ImageType.Transparent){
			TransparentPixel[][] imageTransparent = new TransparentPixel[height][width];
			imageData = imageTransparent;
		}
