...
public class PixelMapPlus extends PixelMap implements ImageOperations
{
	...
	public void rotate(int x, int y, double angleRadian)
	{
		...
		// variable qui permet d'eviter la repetion et rend lisible le calcul.
    ...
    // on sauvegarde les anciennces caracteristiques du tableau
		...
		// On effectue le redimensionnement de l'image
		...
		for(int i = 0; i < h; i++)
		 {
			for(int j = 0; j < w; j++)
			 {
				// Si le pixel est dans l'intervalle du tableau d'origine alors on le met dans image data
	...
	public void translate(int rowOffset, int colOffset)
	{
		...
		for(int i = 0 ; i<this.getHeight(); i++)
		{
			for (int j = 0 ; j < this.getWidth(); j++)
			{
				// On effectue les calculs dans des variables pour rendre lisible le programme
				int x = i - rowOffset;
				int y = j - colOffset;
    ...
    // On cree un tableau temporaire
		...
		if (col0 < 0 ||  row0 < 0)
			...
			// On parcourt tout le tableau  du debut jusqu'a ca sa nouvelle taille
		...
		// On parcourt le tableau entierement
		for(int i=0; i < height; i++ )
        {
            for(int j = 0; j <width; j++ )
            {
	...
	/*
	* Interchange les pixels de maniere verticale
	*( Pour une colonne donnee, on interchange les lignes )
	*/
